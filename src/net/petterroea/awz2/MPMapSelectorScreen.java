...
public class MPMapSelectorScreen extends Screen {
	public int yoffset = 0;
	int port = 1994;
	String name = "";
	public MPMapSelectorScreen(Game thegame, int port, String name) {
		super(thegame);
		this.name = name;
		this.port = port;
	...
	public void mouseReleased(MouseEvent arg0) {
		if(arg0.getY() > 150)
		{
			...
			for(int i = 0; i < MediaManager.maps.size(); i++)
			{
				...
				{
					//Start multiplayer
					ServerSideConnection conn = new ServerSideConnection(port);
					conn.start();
					game.screen = new MpGameScreen(conn, name, game, MediaManager.maps.get(i));
					return;
				}
			}
		}
