	...
	private void sendPackets()
	{
		for(int i = 0; i < clients.size(); i++)
		{
			for(int a = 0; a < clients.get(i).out.size(); a++)
			{
				...
				System.out.println("(Serverside)Sending " + clients.get(i).out.get(a).prefix + " " + clients.get(i).out.get(a).getRaw() + ".");
			}
		...
		for(int i = 0; i < out.size(); i++)
		{
			for(int a = 0; a < clients.size(); a++)
			{
				...
				System.out.println("(Serverside)Sending " + out.get(i).prefix + " " + out.get(i).getRaw() + ".");
			}
		}
		out.clear();
	...
	private void readIncomingMessages() {
		try {
			...
			if (nbytes == -1) { 
				...
				for(int a = 0; a < clients.size(); a++)
				{
					if(clients.get(a).socket.equals(channel))
					{
						id = a;
						break;
					}
				...
				try{
					Packet packet = new Packet(line.split("\\s+"));
					packet.senderId = id;
					in.add(packet);
					} catch(Exception e)
					{
	...
	private void acceptNewPeeps()
	{
		if(clients.size() < 2)
		{
			try {
			    ...
			    System.out.println("There was " + clients.size() + " clients connected");
